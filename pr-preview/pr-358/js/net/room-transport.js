// ============================================================
// RoomTransport — generic WebRTC room (PeerJS P2P + Cloudflare relay)
// ============================================================
//
// The game-agnostic transport extracted from network-manager.js (#318 Step 2,
// the @usersfirst/room seam). It owns room join-by-code, relay primary + P2P
// upgrade + automatic fallback, reconnect/backoff, the HEARTBEAT keepalive, and
// media (camera/mic) calls. It sends/receives OPAQUE bytes — `send(bytes)` out,
// `onMessage(bytes)` in — and knows nothing about bikes. NetworkManager extends
// this and adds the game codec + protocol. HEARTBEAT is the transport's own
// keepalive (the one reserved message type it understands).
// ============================================================
...
export class RoomTransport {
  constructor() {
    ...
    this.lastPingTime = 0;
    this.pingMs = 0;
    // Opaque inbound payload callback — set by the game adapter. Receives every
    // non-keepalive frame as raw bytes; the transport never interprets them.
    this.onMessage = null;
  ...
  _handleMessage(data) {
    ...
    // Everything else is opaque payload — hand it to the game adapter.
    if (this.onMessage) this.onMessage(bytes);
  }
